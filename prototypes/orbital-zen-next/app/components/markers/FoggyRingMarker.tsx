...
export default function FoggyRingMarker({ radius, isCelebrating = false, isHovering = false }: FoggyRingMarkerProps) {
  // Adjust colors and opacity based on state
  const baseColor = isCelebrating
    ? DEFAULT_CONFIG.celebratingColor
    : DEFAULT_CONFIG.normalColor;

  const baseOpacity = isCelebrating
    ? 0.35  // Brighter when celebrating
    : isHovering
    ? 0.20  // Brighter when hovering
