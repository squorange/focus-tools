...
export function validateXYZ(data: Type): ValidationResult {
  ...
  if (!data.requiredField) {
    errors.push(
      `Missing requiredField. This will cause [problem]. ` +
      `Fix by [solution].`
    );
  }
  ...
  if (data.field !== expectedValue) {
    warnings.push(
      `Field has unexpected value. May indicate [issue].`
    );
  }
...
const value = data.persistedValue !== undefined
  ? data.persistedValue
  : calculateFallback(); // Safe fallback using correct logic
...
*These principles are living documentation. Update them as we learn, but never silently ignore them.*
